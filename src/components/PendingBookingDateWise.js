...
function PendingDateWise() {
 ...
 if (fromDate && toDate) {
    ...
    var cfd = fd.toISOString();
    const td = new Date(toDate + ' 23:59:59');
    var ctd = td.toISOString();
 }
  const formData ={
    fromDate: cfd,
    toDate: ctd
    ...
    <table className="w-full">
     ...
     <tr className="border w-full">
      ...
      <tr className="border w-full">
        ...
        <td className="px-3 py-2">{data.paymentMode ? data.paymentMode : 'Pending'}</td>
      </tr>
       <tr className="border w-full">
  ...
  useEffect(() => {
    const result = bookingList?.filter((book) => {
      return (
        ...
        book.dateTime?.toLowerCase().match(search.toLocaleLowerCase())
      );
    });
  ...
  return (
    <div className="flex items-center justify-center">
      ...
      <div className="flex justify-between items-center boton">
       ...
       <Link to='/complete-booking' className="bg-gray-200 w-full text-center cursor-pointer  border-gray-300 hover:border hover:border-gray-300 font-bold py-2">Complete Booking </Link>
       <Link to='/pending-booking' className="bg-gray-200 w-full text-center cursor-pointer border border-gray-300 hover:border hover:border-gray-300 font-bold py-2">Pending Booking </Link>
       <Link to='/cancel-booking' className="bg-gray-200 w-full text-center cursor-pointer hover:border hover:border-gray-300 font-bold py-2">Cancel Booking </Link>
       </div>
        <DataTable
          columns={columns}
          data={searchFilter}
          ...
          subHeaderComponent={
            ...
            <div className="w-full md:flex justify-between items-center px-3">
                <input
              type="text"
            ...
            />
            <button onClick={dateSubmit} className="border rounded px-2 ml-2 bg-gray-200 shadow-xl text-blue-800"> Search</button>
                </div>
           
            </div>
            <div className="flex justify-center itemse-center">
            <button id='btn1' className="border rounded px-3 mx-3 hover:bg-gray-200" 
            onClick={(e) => {
              const dayData ={
                day: e.target.value,
                bookingStatus: 'PENDING'
              }
              axios.post(baseUrl +  'api/v1/admin/dayBookingList',dayData,{
                headers:{
                  token: localStorage.getItem('token')
                }
              }).then((respo) => {
                if (respo.data.responseCode === 200) {
                  setBookingList(respo.data.result)
                  setSearchFilter(respo.data.result)
                }else{
                  alert('No pending booking in last 7 days')
                }
              })
              }} value='7'>Last 7 Day</button>
            <button id='btn2' className="border rounded px-3 mx-3 hover:bg-gray-200" 
            onClick={(e) => {
              const dayData ={
                day: e.target.value,
                bookingStatus: 'PENDING'
              ...
              axios.post(baseUrl +  'api/v1/admin/dayBookingList',dayData,{
                ...
                if (respo.data.responseCode === 200) {
                  ...
                  alert('No pending booking in last 15 days')
                }
              })
              }} value='15'>Last 15 Day</button>
            <button id='btn3' className="border rounded px-3 mx-3 hover:bg-gray-200" 
            onClick={(e) => {
              const dayData ={
                day: e.target.value,
                bookingStatus: 'PENDING'
              ...
              axios.post(baseUrl +  'api/v1/admin/dayBookingList',dayData,{
                ...
                if (respo.data.responseCode === 200) {
                  ...
                  alert('No pending booking in last 30 days')
                }
              })
              }} value='30'>Last 30 Day</button>
            </div>
            </div>
          }
          subHeaderAlign="left"
...
export default PendingDateWise;
